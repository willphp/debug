...
class DebugBuilder {
	...
	public function getTrace() {
		...
		$end_time = round((microtime(true) - RUN_TIME) , 4);
		$errno = '';
	...
	protected function parseTrace() {
		...
		$route = is_callable($get_route)? call_user_func($get_route) : '';		
		if ($route) {			
			$this->items['base']['路由参数'] = $route;
		}		
		$end_memory = memory_get_usage() - RUN_MEMORY;
		$end_time = round((microtime(true) - RUN_TIME) , 4);
		...
		$this->items['base']['内存开销'] = number_format($end_memory/1024,2).' KB';
		$this->items['base']['调试统计'] = '文件：'.count($files).'('.number_format($filesize/1024, 2).' KB)';
		$this->items['base']['运行时间'] = $end_time.'s at '.date('Y-m-d H:i:s', $_SERVER['REQUEST_TIME']).' <a href="http://www.113344.com" style="color:green;" target="_blank">WillPHP</a>';
